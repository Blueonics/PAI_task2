...
def ece(predicted_probabilities: np.ndarray, labels: np.ndarray, n_bins: int = 30) -> float:
    ...
    predictions = np.argmax(predicted_probabilities, axis=1) #pick the predicted class
    ...
    probs_as_bin_num = np.digitize(prediction_confidences, bin_upper_edges) # split bins
